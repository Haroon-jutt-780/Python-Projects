...
def display_welcome_banner() -> None:
    clear_screen()
    print("=" * 70)
    print("        📰 UAF LATEST NEWS & ANNOUNCEMENTS SCRAPER 📰        ")
    print("=" * 70)
    print(" Purpose: Automatically extracts full headlines, dates, times, event")
    print("          locations, and links directly from the UAF news portal.")
    print("=" * 70 + "\n")
...
def get_news_count() -> int:
    while True:
        try:
            user_input = input("👉 Enter the number of news articles to scrape: ").strip()
...
def run(playwright: Playwright) -> None:
    ...
    try:
        print("⏳ Fetching news catalog in background...")
        page.goto("https://web.uaf.edu.pk/News/AllNews")
        html_data = page.content()
        soup = BeautifulSoup(html_data, "lxml")

        #Gets user input for news articles
        user_input = get_news_count()
        ...
        # GUARANTEED to run even if the user hits Ctrl+C or get_input() crashes
        context.close()
        browser.close()
